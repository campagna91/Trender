	...
	<body> 
		<?
			...
			if(!isset($_GET['id'])) {
				...
				<a id="addActor" class="btn-floating btn-large waves-effect waves-light red modal-trigger" data-target="actorsInsertion"><i class="material-icons">add</i></a> <?

				// Modal insertion window
				include_once 'cgi-bin/__moduleActorsInsertion.php';
			} else {
				if(!esiste($_GET['id'])) { ?>
				...
				<? } else {
					...
					<? include_once '__moduleActorsUpdate.php' ?>

					<div class="row">
						<? 

							// Module required to perform operation on requirement's child
							include_once '__moduleActorsDerivative.php';

							// Module required to perform operation on requirement's usecases combine
							include_once '__moduleActorsUsecase.php';

						?>
					</div>

					<!-- Perform a delete of requirement -->
					<a id="actorDelete" class="waves-effect waves-light col s12 red btn-large">Delete actor</a>
