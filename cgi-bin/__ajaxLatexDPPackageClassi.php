...
function stampaPackage($dad) {
  ...
  {
    if(preg_match("/(socket)|(fabric)|(jspdf)|(impress)$/", $v[0]))
        continue;
    ...
    //CHECK DIMENSION----------------------------------------
    if($v[0] == "premi::client::editor::export" || $v[0] == "premi::client::editor::overview" || $v[0] == "premi::client::core::home" || $v[0] == "premi::client::editor::container" || $v[0] == "premi::client::editor::menu" || $v[0] =="premi::client::editor::nav" || $v[0] =="premi::server::model" || $v[0] =="premi::server::controller" || $v[0] =="premi::client::editor" || $v[0] =="premi::client::player")
    ...
    {
        if(substr($vInherit[0],0,1) == "_")
          $vTitoloCheckUnder = substr($vInherit[0],1);
        else
            $vTitoloCheckUnder = $vInherit[0];
        fwrite($GLOBALS['definizione'], "\\item \\pkg{".$vTitoloCheckUnder."}: ".$vInherit[2]);
        $i--;
        if($i) fwrite($GLOBALS['definizione'], ";\n");
        else fwrite($GLOBALS['definizione'], ".\n");
    }
    ...
    if(mysqli_num_rows($qCheckClass))
    {
      ...
      while($vCheckClass = $qCheckClass->fetch_array())
      {
        ...
        if(!preg_match("/(socket)|(fabric)|(Menus)|(jsPDF)|(jmpress)|(CoreController)|(UsersController)|(ProfileController)|(_AuthenticationController)|(AuthorizationController)$/", $vCheckClass[0]) && !preg_match("/(premi::client::users)|(premi::server::model)|(premi::server::view)$/", $v[0]))
        {
            fwrite($GLOBALS['definizione'], "\\begin{figure}[h!] \n");
            fwrite($GLOBALS['definizione'], "\\centering\n");

            // CHECK DIMENSION--------------------------
            if($vCheckClass[0] == "menu" || $vCheckClass[0] == "overview" || $vCheckClass[0] == "player" || $vCheckClass[0] == "Presentation")
                fwrite($GLOBALS['definizione'], "\\includegraphics[width=1\\textwidth]{");
            else
                fwrite($GLOBALS['definizione'], "\\includegraphics[scale=0.4]{");
            $arg = explode("::",$v[0]);
            ...
            fwrite($GLOBALS['definizione'], "img/premi/".$arg[0]."__".$arg[1]."__".$arg[2]."_".$vTitoloCheckUnder);
            fwrite($GLOBALS['definizione'], "}\n");
            fwrite($GLOBALS['definizione'], "\\caption{Diagramma della classe \cls{".$vCheckClass[3]."::".$vTitoloCheckUnder."}}\n");
            fwrite($GLOBALS['definizione'], "\\end{figure}\n");
        }
        ...
        if(mysqli_num_rows($qAttribute))
        {
          ...
          {
            fwrite($GLOBALS['definizione'], "\\item \\mmybox{\\attribute{".$vAttribute[1]."} : \\type{".$vAttribute[2]."}}\n\n ".$vAttribute[3].".\n");
          }
          fwrite($GLOBALS['definizione'], "\\end{itemize}\n");
        }
        ...
        if(mysqli_num_rows($qMethod))
        {
          ...
          while($vMethod = $qMethod->fetch_array())
          {
            if(substr($vMethod[1],0,1)=='$')
                ...
                $nomeMetodo = $vMethod[1];
            fwrite($GLOBALS['definizione'], "\\item \\mmybox{\\method{".$nomeMetodo."}(");
            if($vMethod[2]!="")
            {
              ...
              {
                $expl = explode(":",$arg[$i]);
                fwrite($GLOBALS['definizione'], "\\argument{".$expl[0].":\\enskip\\type{".$expl[1]."}}");
                if(($dim-$i-1)!=1)
                  fwrite($GLOBALS['definizione'], ", ");
              }
            }
            fwrite($GLOBALS['definizione'], "):\\enskip\\type{".$vMethod[3]."}}\n\n".$vMethod[4].".");
            if($vMethod[2]!="")
            {
              fwrite($GLOBALS['definizione'], "\n\n \\textbf{Argomenti}:");
              fwrite($GLOBALS['definizione'], "\\begin{itemize}\n");
              ...
              {
                $expl = explode(":",$arg[$i]);
                fwrite($GLOBALS['definizione'], "\\item \\mmybox{\\argument{".$expl[0]."} : \\type{".$expl[1]."}} ".$expl[2].".");
              }
              fwrite($GLOBALS['definizione'], "\\end{itemize}\n");
            }
          }
